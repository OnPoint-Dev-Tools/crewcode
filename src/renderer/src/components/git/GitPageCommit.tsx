...
export function GitPageCommit({ branch, stagedCount, onCommit, onPush, onPull, onFetch, onSync }: GitPageCommitProps) {
  ...
  const commit = (mode: CommitMode) => {
    if (!canCommit) return
    onCommit?.({ message, amend, push: mode === 'push', sync: mode === 'sync' })
    setMessage('')
    setAmend(false)
  }
  ...
  const items = [
    ...
    { label: 'commit & push', icon: 'gitCommit' as const, run: () => commit('push'), disabled: !canCommit },
    { label: 'commit & sync', icon: 'refresh' as const, run: () => commit('sync'), disabled: !canCommit },
    ...
    <section className="git-page-commit" aria-label="Commit changes">
      ...
      <div className="git-page-commit-foot">
        ...
        <div className="git-page-commit-actions">
          <button className="gs-btn primary" disabled={!canCommit} onClick={() => commit('plain')}>
