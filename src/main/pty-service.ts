...
export class PtyService {
  ...
  create(opts: PtyCreateOpts): { ok?: boolean; error?: string; attached?: boolean; pid?: number; buffer?: string; cwd?: string; shell?: string } {
    const { paneId, cwd, cols = 80, rows = 24, shell, argv = [], env = {} } = opts
    ...
    const ptyEnv: Record<string, string> = {
      ...(process.env as Record<string, string>), ...env, SHELL: exe, TERM: 'xterm-256color',
      COLORTERM: 'truecolor', TERM_PROGRAM: 'CrewCode', TERM_PROGRAM_VERSION: process.env.npm_package_version ?? '1.0.0', FORCE_COLOR: '1',
    }
