...
export async function startHubServer(options: HubServerOptions): Promise<RunningHubServer> {
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (request.method === 'POST' && pathname === '/api/v1/hub/machines/heartbeat') {
        ...
        if (machine.disabledAt !== null) { sendJson(response, 423, { error: 'machine is disabled by its Hub owner' }); return }
        const body = await readJson(request)
        const platform = boundedString(body.platform, 'platform', 80, true)
        const version = boundedString(body.version, 'version', 80, true)
      ...
      const enabledMatch = request.method === 'POST' ? pathname.match(/^\/api\/v1\/hub\/machines\/([a-f0-9]{32})\/enabled$/) : null
      if (enabledMatch) {
        const session = currentSession(request)
        if (!session) { sendJson(response, 401, { error: 'valid Hub session required' }); return }
        if (!validCsrf(request, session)) { sendJson(response, 403, { error: 'valid CSRF token required' }); return }
        const body = await readJson(request)
        if (typeof body.enabled !== 'boolean') { sendJson(response, 400, { error: 'enabled must be a boolean' }); return }
        const machine = store.setMachineEnabled(session.userId, enabledMatch[1], body.enabled, now())
        if (!machine) { sendJson(response, 404, { error: 'enrolled machine not found' }); return }
        if (!body.enabled) {
          brainSockets.get(enabledMatch[1])?.close(4003, 'machine disabled')
          for (const [connectionId, connection] of relayConnections) {
            if (connection.machineId !== enabledMatch[1]) continue
            connection.browser.close(4003, 'machine disabled')
            relayConnections.delete(connectionId)
          }
        }
  ...
  server.on('upgrade', (request, socket, head) => {
    ...
    if (protocol === HUB_BRAIN_RELAY_PROTOCOL) {
      ...
      if (machine.disabledAt !== null) { socket.write('HTTP/1.1 403 Forbidden\r\nConnection: close\r\n\r\n'); socket.destroy(); return }
