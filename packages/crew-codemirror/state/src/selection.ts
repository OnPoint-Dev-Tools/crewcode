...
// - 1 bit for the 'undecided' flag, which is used for MacOS style
//   range selections that will pick a head and anchor side based on
//   which direction the next shift-cursor-motion goes.
...
export class SelectionRange {
  ...
  get undirectional(): boolean {
    return (this.flags & RangeFlag.Undirectional) > 0
  }
  ...
  /// @internal
  static create(from: number, to: number, flags: number, goalColumn: number | undefined) {
...
export class EditorSelection {
  ...
  static range(anchor: number, head: number, goalColumn?: number, bidiLevel?: number, assoc?: number) {
    ...
    if (assoc) flags |= assoc < 0 ? RangeFlag.AssocBefore : RangeFlag.AssocAfter
    return head < anchor ? SelectionRange.create(head, anchor, flags | RangeFlag.Inverted, goalColumn)
      : SelectionRange.create(anchor, head, flags, goalColumn)
  }

  /// Create an [undirectional](#state.SelectionRange.undirectional)
  /// selection range.
  static undirectionalRange(from: number, to: number) {
    return SelectionRange.create(from, to, RangeFlag.Undirectional, undefined)
  }
