...
function extendSel(target: CommandTarget, forward: boolean, how: (range: SelectionRange) => SelectionRange): boolean {
...
export const selectCharForwardLogical: StateCommand =
  target => extendSel(target, true, range => byCharLogical(target.state, range, true))
/// Move the selection head one character backward by logical string
/// index order.
export const selectCharBackwardLogical: StateCommand =
  target => extendSel(target, false, range => byCharLogical(target.state, range, false))
...
export const selectLineStart: Command = view =>
  extendSel(view, false, range => EditorSelection.cursor(view.lineBlockAt(range.head).from))
/// Move the selection head to the end of the line.
export const selectLineEnd: Command = view =>
  extendSel(view, true, range => EditorSelection.cursor(view.lineBlockAt(range.head).to))
...
export const selectLine: StateCommand = ({state, dispatch}) => {
  let ranges = selectedLineBlocks(state).map(
    ({from, to}) => EditorSelection.undirectionalRange(from, Math.min(to + 1, state.doc.length)))
