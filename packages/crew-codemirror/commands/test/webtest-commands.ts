...
import {cursorSubwordForward, cursorSubwordBackward, cursorLineDown, cursorLineUp,
        selectLineDown, selectLineUp, selectCharLeft, selectCharRight} from "@codemirror/commands"
import ist from "ist"
...
describe("commands", () => {
  ...
  describe("selectCharLeft", () => {
    it("selects backwards left-to-right text", () => {
      testCmd("ab|cd", "a<b>cd", selectCharLeft)
    })

    it("selects forward in right-to-left text", () => {
      testCmd("ال|شفرة", "ال<ش>فرة", selectCharLeft, EditorView.theme({".cm-content": {direction: "rtl"}}))
    })

    it("expands the start of an undirectional selection", () => {
      testCmd("a<bc>d", "<abc>d", selectCharLeft)
    })
  })

  describe("selectCharRight", () => {
    it("expands the end of an undirectional selection", () => {
      testCmd("a<bc>d", "a<bcd>", selectCharRight)
    })
  })
})
