...
const CollapsedRanges = StateField.define<DecorationSet>({
  ...
  update(deco, tr) {
    ...
    if (deco.size && tr.state.field(ChunkField) != tr.startState.field(ChunkField, false)) {
      let sideA = tr.state.facet(mergeConfig).side == "a", touchingChange: number[] = []
      for (let chunk of tr.state.field(ChunkField)) {
        deco.between(sideA ? chunk.fromA : chunk.fromB, sideA ? chunk.toA : chunk.toB,
                     from => { touchingChange.push(from) })
      }
      if (touchingChange.length) deco = deco.update({filter: from => touchingChange.indexOf(from) < 0})
    }
    return deco
  },
  provide: f => EditorView.decorations.from(f)
