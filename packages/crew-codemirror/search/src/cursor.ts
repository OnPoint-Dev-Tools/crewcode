...
/// document.
export class SearchCursor implements Iterator<{from: number, to: number, precise: boolean}>{
  ...
  value = {from: 0, to: 0, precise: false}
  /// Whether the end of the iterated region has been reached.
  done = false
  private matches: {from: number, index: number, precise: boolean}[] = []
  private buffer = ""
  private bufferPos = 0
  ...
  private match(code: number, pos: number, posPrecise: boolean, end: number, endPrecise: boolean) {
    let match: null | {from: number, to: number, precise: boolean} = null
